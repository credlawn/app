...
class TodayDetailsScreen extends StatefulWidget {
  ...
  const TodayDetailsScreen({super.key, required this.employees});

  @override
  _TodayDetailsScreenState createState() => _TodayDetailsScreenState();
}
...
class _TodayDetailsScreenState extends State<TodayDetailsScreen> {
  ...
  Future<void> _shareScreenshot() async {
    try {
      ...
      if (imageBytes != null) {
        ...
        final fileName = 'manager_dashboard_today_$timestamp.png';
        final file = File('${tempDir.path}/$fileName');

        await file.writeAsBytes(imageBytes);
        await Share.shareXFiles(
          [XFile(file.path)],
          text: 'Manager Dashboard Today Report - ${DateFormat('MMM dd, yyyy').format(DateTime.now())}',
          subject: 'Manager Dashboard Today Screenshot',
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Today Details', style: GoogleFonts.poppins(color: Colors.white, fontSize: 18)),
